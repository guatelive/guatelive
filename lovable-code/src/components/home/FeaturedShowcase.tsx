    ...
    return (
        <section className="mx-auto mt-14 max-w-6xl px-4 sm:px-6">
            ...
            <div className="mb-5 flex items-end justify-between gap-4">
                <div>
                    <p className="flex items-center gap-1.5 text-xs uppercase tracking-[0.18em] text-accent">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                <Link
                    to={current.href}
                    key={current.id}
                    className="group relative col-span-1 overflow-hidden rounded-3xl lg:col-span-2 animate-fade-in"
                >
                    <div className="relative aspect-[4/5] w-full overflow-hidden sm:aspect-[16/10] lg:aspect-[5/4]">
                        ...
                        <div className="absolute inset-x-5 top-5 flex items-start justify-between gap-3">
                            ...
                            <span
                                className={`hidden rounded-full px-3 py-1.5 text-xs font-semibold shadow-lg sm:inline ${accentBg[current.accent]}`}
                            >
                                {current.offer}
                            </span>
                        </div>
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-8">
                            ...
                            <div className="mt-5 flex flex-wrap items-center gap-3">
                                <span
                                    className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium shadow-lg ${accentBg[current.accent]}`}
                                >
                                    {current.cta}
            ...
            <div className="mt-5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    {featured.map((f, i) => (
                        <button
                            ...
                            className={`h-1.5 rounded-full transition-all ${i === index ? "w-8 bg-primary" : "w-2.5 bg-border hover:bg-muted-foreground"
                                }`}
                        />
                    ))}
                </div>
                ...
                <Link
                    to="/promos"
                    className="hidden text-sm font-medium text-primary hover:text-accent sm:inline"
                >
